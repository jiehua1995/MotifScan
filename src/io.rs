...
/// Source format for an input record, used to distinguish FASTA from FASTQ parsing paths.
...
/// Unified read record structure that hides the format differences between FASTA and FASTQ.
#[derive(Debug, Clone)]
pub struct Record {
    pub id: String,
    pub seq: Vec<u8>,
...
/// Streaming reader exposed to the scanner, internally dispatching to the format-specific parser.
...
/// Input progress state tracking bytes read and total bytes for progress reporting.
#[derive(Clone)]
...
/// One immutable progress snapshot so callers do not need to touch the internal atomics directly.
...
/// Internal parser enum that switches between format-specific state machines.
enum ReaderImpl {
    Fasta(FastaReader),
    Fastq(FastqReader),
}

/// Opens the input file, auto-detects FASTA/FASTQ format, and returns a unified `RecordReader`.
...
impl RecordReader {
    /// Returns the source format handled by this reader.
    pub fn source_format(&self) -> SourceFormat {
        self.source_format
    }

    /// Pulls the next chunk of records; this is the natural batch boundary used by the parallel scanner.
    ...
    /// Returns a snapshot of the current input-reading progress.
    pub fn progress_snapshot(&self) -> ProgressSnapshot {
        self.progress.snapshot()
    }
}

// Opens the underlying input stream and wraps it with gzip decoding and byte counting when needed.
...
impl InputProgress {
    // Converts the internal atomics into a stable snapshot and guarantees total bytes is at least 1 to avoid divide-by-zero issues.
...
/// Lightweight wrapper that adds byte-counting behavior to any underlying reader.
...
impl<R> CountingReader<R> {
    /// Creates a new byte-counting reader wrapper.
...
impl<R: Read> Read for CountingReader<R> {
    /// Forwards the underlying `read` call and adds the consumed byte count to the progress counter.
...
impl<R: BufRead> BufRead for CountingReader<R> {
    /// Exposes the underlying buffer directly without copying data here.
    fn fill_buf(&mut self) -> std::io::Result<&[u8]> {
        self.inner.fill_buf()
    }

    /// Increments the read-byte counter when the buffer is consumed.
...
// Detects FASTA/FASTQ primarily from the file extension and falls back to the first non-whitespace byte when needed.
...
// Performs a fast format guess from the filename suffix to avoid probing file contents when possible.
...
// Reads one line and strips trailing newline characters so the FASTA/FASTQ state machines can process clean content.
...
// Converts one text sequence line into an uppercase byte vector.
...
// Appends a trimmed, uppercased sequence fragment into the destination buffer, mainly for multiline FASTA parsing.
...
/// FASTA reader that maintains a pending header and line buffer to implement sequential state-based parsing.
struct FastaReader {
...
impl FastaReader {
    /// Creates a FASTA reader with its internal parser state initialized.
    ...
    /// Reads the next FASTA record, supporting multiline FASTA and stopping when the next header is encountered.
...
/// FASTQ reader that parses records using the standard four-line layout.
struct FastqReader {
    reader: Box<dyn BufRead>,
    line_buffer: String,
}
...
impl FastqReader {
    ...
    /// Reads the next FASTQ record and decodes quality scores from ASCII Phred+33 into numeric values.
    fn next_record(&mut self) -> Result<Option<Record>> {
        let bytes = loop {
            let bytes = read_line_trimmed(self.reader.as_mut(), &mut self.line_buffer)?;
            if bytes == 0 {
...
mod tests {
    ...
    #[test]
    // Verifies that the FASTA reader correctly joins multiline sequence content into one record.
    ...
    #[test]
    // Verifies that the FASTQ reader uppercases sequence data and decodes qualities as Phred+33 values.
    ...
    #[test]
    // Verifies that gzip-compressed input can also be read correctly through the same unified reader interface.
